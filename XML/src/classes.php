    ...
    static public function getCaptcha()
    {
        if (!isset($_SESSION[CAPTCHA_SESSION_VAR]) || $_SESSION[CAPTCHA_SESSION_VAR] === NULL) {
            ...
            for ($i = 0; $i < 6; $i++) {
                ...
                $_SESSION[CAPTCHA_SESSION_VAR] .= $str;
            }
        }
...
class Utilizator
{
    ...
    public function __construct($user_type, $username)
    {
...
class Administrator extends Utilizator
{
    $administrator = new Utilizator('admin', 'NumeAdmin');
}

class Membru extends Utilizator
{
    $utilizator = new Utilizator('user', 'NumeUtilizator');
}
?>
